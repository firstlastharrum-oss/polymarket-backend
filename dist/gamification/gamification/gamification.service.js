...
let GamificationService = class GamificationService {
    ...
    async getLeaderboard(type, limit = 10) {
        let orderBy = { reputationScore: 'desc' };
        switch (type) {
            case 'earnings':
        ...
        const visible = profiles.filter((p) => {
            const vis = String(p.user?.setting?.profile_visibility || 'public').toLowerCase();
            return vis !== 'private';
    ...
    async incrementBetCreation(userId) {
    ...
    async updateResolutionStats(userId, isWin, profitOrLoss) {
    ...
    async getPublicProfile(userId) {
        const profile = await this.getUserStats(userId);
        const vis = String((profile.user?.setting || {})?.profile_visibility || 'public').toLowerCase();
        if (vis === 'private') {
            return { success: true, visibility: 'private' };
        }
        const username = profile.user?.username || `user-${userId}`;
        ...
        const createdAt = profile.user?.createdAt || null;
        const createdMarkets = await this.prisma.match.findMany({
            where: { OR: [{ betA: { creatorId: userId } }, { betB: { creatorId: userId } }] },
            include: { pool: true },
        });
        const activeMarkets = createdMarkets.filter((m) => {
            const s = String(m.pool?.status || '');
            return ['LIVE', 'CLOSED', 'PROVISIONAL', 'DISPUTED', 'RESOLUTION_OPEN', 'RESOLUTION_CALCULATION'].includes(s);
        }).length;
        const resolvedMarkets = createdMarkets.filter((m) => {
            const s = String(m.pool?.status || '');
            return ['RESOLVED', 'PAID_OUT', 'FINALIZED', 'SETTLED'].includes(s);
        }).length;
        const marketVolumeGenerated = createdMarkets.reduce((acc, m) => acc + Number(m.pool?.totalPoolAmount || 0), 0);
        const commissionEarned = await this.prisma.creatorEarnings.aggregate({
        ...
        const finalizedVotes = votes.filter((v) => ['RESOLVED', 'PAID_OUT', 'FINALIZED', 'SETTLED'].includes(String(v.pool?.status || '')));
        const totalResolutions = finalizedVotes.length;
        const correctResolutions = finalizedVotes.filter((v) => String(v.outcome || '').toUpperCase() === String(v.pool?.outcome || '').toUpperCase()).length;
        const incorrectResolutions = totalResolutions - correctResolutions;
        const resolutionStakeTotal = finalizedVotes.reduce((acc, v) => acc + Number(v.amount || 0), 0);
        const rewardEvents = await this.prisma.eventLog.findMany({
            where: { type: 'OracleRewardPaid' },
            orderBy: { createdAt: 'desc' },
            take: 500,
        });
        const oracleRewards = rewardEvents.reduce((acc, e) => {
        ...
        const finalizedPositions = positions.filter((p) => ['RESOLVED', 'PAID_OUT', 'FINALIZED', 'SETTLED'].includes(String(p.pool?.status || '')));
        const bettingVolume = finalizedPositions.reduce((acc, p) => acc + Number(p.amount || 0), 0);
        const uniqueMarketIds = new Set();
        for (const p of finalizedPositions)
            uniqueMarketIds.add(Number(p.poolId));
        for (const v of finalizedVotes)
            uniqueMarketIds.add(Number(v.poolId));
